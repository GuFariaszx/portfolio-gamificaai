import { Actor, Animation, Collider, CollisionContact, CollisionType, Color, Engine, Graphic, Keys, Side, SpriteSheet, Vector, vec } from "excalibur";
...
export class Player extends Actor{
    ...
    onInitialize(engine: Engine<any>): void {
        ...
        engine.input.keyboard.on("release", (event) => {
            ...
            // PAra movimentar lateral ao soltar as teclas de movimentação lateral
            if(
                event.key == Keys.A ||
                event.key == Keys.Left ||
                event.key == Keys.D ||
